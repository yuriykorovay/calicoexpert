...
calicoctl -h &> /dev/null || (echo "Downloading calicoctl"; curl -Lo "`echo ~`/.local/bin/calicoctl" https://github.com/projectcalico/calicoctl/releases/download/v3.20.6/calicoctl-linux-amd64 )
kubectl -h &> /dev/null || (echo "Downloading kubectl"; curl -Lo "`echo ~`/.local/bin/kubectl" https://dl.k8s.io/release/v1.30.2/bin/linux/amd64/kubectl )
kops -h &> /dev/null || (echo "Downloading kOps"; curl -Lo "`echo ~`/.local/bin/kops" https://github.com/kubernetes/kops/releases/download/v1.29.0/kops-linux-amd64 )
...
eksctl -h &> /dev/null || (echo "Downloading eksctl"; curl -Lo /tmp/eksctl.tgz "https://github.com/weaveworks/eksctl/releases/download/v0.183.0/eksctl_$(uname -s)_amd64.tar.gz" && tar xzvf /tmp/eksctl.tgz > /dev/null && mv eksctl ~/.local/bin/eksctl )
